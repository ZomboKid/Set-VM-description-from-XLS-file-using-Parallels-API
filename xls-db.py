...
# ----------------------------------------------------------------------------
parser = argparse.ArgumentParser(usage="The utility reads from XLS names and \
description of VM and get from MariaDB name of its host.\n\rBefore using the \
utility, first configure the configuration YAML file ./xls-db_conf.yaml\n\r\n\
\rFIRST - run with key -get - utility reads from XLS VMs and description; \
then reads from MariaDB VM Hosts and write hosts.yaml (this file needs for \
-set key).\n\rSECOND - to set description to R-Management Console you must \
set the credentials in the file hosts.yaml and run the utility with key -set\
\n\rAlso the utility does a backup of the VM description to a file \
backup_description.yaml", formatter_class=argparse.RawTextHelpFormatter)
parser.add_argument("-get", "--get_description", action="store_true",
                    help="get description from XLS")
parser.add_argument("-set", "--set_description", action="store_true",
                    help="set description to R-Management Console")
...
def parse_xls(XLS_FILE, VM_NAME_SHEET, VM_NAME_COLUMN, VM_DESCRIPTION_SHEET,
    ...
    mariadb_connection = mysql.connector.connect(user='USERNAME',
                                                 database='DBNAME')
    ...
    for j in heap_cfg:
        for i in hosts_cfg:
            if j['HOST'] == i['HOST']:
                if j['DESC'] is None:
                    j['DESC'] = ""
                api_vm_change_description(j['HOST'], i['USER'], i['PASSWORD'],
                                          j['VM'], j['DESC'])
                print "For VM", j['VM'], "setting description",\
                      j['DESC'].encode("utf_8"), "OK"
                break
    heap_file.close()
    hosts_file.close()
# ----------------------------------------------------------------------------
...
def main():
    ...
    if args.get_description:
        parse_xls(XLS_FILE, VM_NAME_SHEET, VM_NAME_COLUMN,
                  VM_DESCRIPTION_SHEET, VM_DESCRIPTION_COLUMN,
                  ENCODE, HOST_REPRESENT)
    if args.set_description:
        if os.path.exists("./hosts.yaml") and os.path.exists("./heap.yaml"):
            set_desc()
        else:
            print "First run utility with key -get and it is write two files:\
                  ./heap.yaml (with VMs, hosts and description) and \
                  ./hosts.yaml (with credentials to hosts)"
            sys.exit(1)
# ----------------------------------------------------------------------------
